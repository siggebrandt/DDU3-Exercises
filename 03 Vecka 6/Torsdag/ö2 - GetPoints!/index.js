...
class Game extends E {
  ...
  static end () {
    Game.active = false;
    document.querySelector(".grid").style.opacity = .2;
  }
  constructor (grid) {
...
class Grid extends E {
  ...
  constructor (grid) {
    ...
    this.html.addEventListener("keydown", e => {      
      ...
      //     break;
      //   default:
      //     return; // Så att vi inte dekrementerar moves om man trykcer på ett annat tangent.
      // }
      // Game.playerInfo.moves--;

      // De bortkommenterade raderna ovan ersätts med:
      if (e.key.includes("Arrow")) {
        let direction = e.key.substring(5); // Extraherar riktningen
        this.token[`move${direction}`]();   // bracket-notation för metoden, som också anropas, därav parenteserna.
        Game.playerInfo.moves--;
      }
    });
...
class Token extends E {
  ...
  set col (value) {
    if (value < 1) value = Grid.nCols;
    if (value > Grid.nCols) value = 1;
  ...
  set row (value) {
    if (value < 1) value = Grid.nRows;
    if (value > Grid.nRows) value = 1;
